...
async def lifespan(app: FastAPI):
    ...
    cleanup_task = asyncio.create_task(cleanup_loop(settings), name="file-cleanup")
    try:
        yield
    finally:
        cleanup_task.cancel()
...
def _prepare_target(data: bytes, token: str, include_thumbnails: bool) -> tuple[Path, list[Any]]:
    ...
    try:
        ...
        detections = face_engine.detect(decoded.bgr)
        if not detections:
...
def _perform_swap(
    ...
    detections = face_engine.detect(target_bgr)
    if not detections:
        raise FaceNotFoundError("沒有偵測到臉")
    if target_face_index is None:
        if len(detections) > 1:
            raise InvalidFaceIndexError("偵測到多張臉，請先選擇要替換的人臉")
        target_face_index = 0
    if target_face_index < 0 or target_face_index >= len(detections):
        raise InvalidFaceIndexError(
            f"選擇的人臉編號無效，可用範圍為 0 到 {len(detections) - 1}"
        )
...
async def list_faces() -> FaceListResponse:
    return FaceListResponse(faces=[_face_response(item) for item in database.list_faces()])
...
async def create_face(
    ...
    try:
        original_path, aligned_path, embedding_path = await _run_inference(
            request, _create_face_assets, data, face_id
        )
...
async def rename_face(face_id: str, payload: RenameFaceRequest) -> FaceResponse:
    clean_name = _clean_name(payload.name)
    try:
        record = database.rename_face(face_id, clean_name, _utc_now())
    except sqlite3.Error as exc:
        raise ApiError(503, "人臉資料庫暫時無法寫入，請稍後再試", "database_error") from exc
    if record is None:
...
async def delete_face(face_id: str) -> Response:
    try:
        record = database.delete_face(face_id)
    except sqlite3.Error as exc:
        raise ApiError(503, "人臉資料庫暫時無法寫入，請稍後再試", "database_error") from exc
    if record is None:
        raise ApiError(404, "找不到指定的人臉", "face_record_not_found")
    await asyncio.to_thread(_delete_face_assets, record)
    return Response(status_code=204)


@app.get("/api/faces/{face_id}/thumbnail", include_in_schema=False)
async def get_face_thumbnail(face_id: str) -> FileResponse:
...
async def get_temp_thumbnail(filename: str) -> FileResponse:
    if not TEMP_THUMBNAIL_PATTERN.fullmatch(filename):
        raise ApiError(404, "找不到暫存縮圖", "file_not_found")
    path = _safe_path(settings.temp_dir, settings.temp_dir / filename)
    return FileResponse(
        path,
        media_type="image/jpeg",
...
async def swap_face(
    ...
    if target_image is not None:
        ...
        target_path = _safe_path(settings.temp_dir, settings.temp_dir / f"{token}.jpg")
        try:
...
async def get_result(filename: str, download: bool = False) -> FileResponse:
    if not RESULT_PATTERN.fullmatch(filename):
        raise ApiError(404, "找不到結果圖片", "file_not_found")
    path = _safe_path(settings.results_dir, settings.results_dir / filename)
    return FileResponse(
        path,
        media_type="image/jpeg",
