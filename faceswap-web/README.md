...
5. 在服務的環境變數同時設定強密碼 `APP_USERNAME` 與 `APP_PASSWORD`，避免公開網址上的人臉庫被他人操作。
6. 保持單一服務副本。此專案使用 SQLite 與本機 volume，不適合讓多個副本同時寫入同一資料庫。
...
| `APP_USERNAME` | 未設定 | 可選的單一使用者 HTTP Basic 帳號；須與密碼同時設定 |
| `APP_PASSWORD` | 未設定 | 可選的單一使用者 HTTP Basic 密碼；請使用長且不重複的密碼 |
...
- **Zeabur 日誌只有 Caddy，沒有 Uvicorn／模型訊息**：這通常表示 Zeabur 沒有使用本專案的 `Dockerfile`。確認 GitHub 中 `Dockerfile` 已提交，且 monorepo 服務的 Root Directory 精確設為 `faceswap-web`，再觸發重新部署。Caddy 顯示 `HTTP/2 skipped because it requires TLS`／`HTTP/3 skipped` 只是內部 `:8080` 使用 HTTP 時的提示，不是 FastAPI 模型服務成功啟動的證明。
...
此專案可用 `APP_USERNAME`／`APP_PASSWORD` 保護整個服務，但只有單一共用帳號，沒有註冊、角色權限或多人資料隔離；未設定時，任何能開啟服務的人都可能看到或修改同一人臉庫。結果與暫存圖會定期刪除，但 `/data/faces` 與 SQLite 會持續保存，管理者應自行備份並依需求刪除。內建「臉部修復」是保守的局部銳化，不是 GFPGAN 等生成式修復模型。`inswapper_128` 不保證身份相似度、自然度或高解析品質，也不應用於任何身分驗證情境。
